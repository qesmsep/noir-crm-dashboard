...
async function testLatestWebhookEvent() {
  console.log('🔍 Testing Latest Webhook Event...\n');

  // Latest webhook event data
  const session = {
    id: 'cs_live_a1mYFcQC5oSIfvzG6Ll8Q9gHOHyuaRI7LQrA3En9o0nBlCifbIimcBAKq6',
    customer: 'cus_Sf6gZAST6HBZX9',
    client_reference_id: null, // This is null in the real event
    customer_details: {
      email: 'hello@skylineandco.com',
      phone: '+19137774488',
      name: 'Tim Wirick'
    },
  ...
  // Step 1: Try to find account by phone first (as per updated logic)
  if (!accountId && session.customer_details && session.customer_details.phone) {
    ...
    const { data: membersByPhone, error: phoneSearchError } = await supabase
      .from('members')
      .select('account_id, member_id, first_name, last_name, status, membership, email')
      .eq('phone', session.customer_details.phone)
      .eq('member_type', 'primary')
      .limit(5); // Get multiple to see all matches
    
    if (phoneSearchError) {
      console.log('❌ Error searching by phone:', phoneSearchError.message);
    } else if (membersByPhone && membersByPhone.length > 0) {
      console.log(`✅ Found ${membersByPhone.length} members with phone:`, session.customer_details.phone);
      membersByPhone.forEach((member, index) => {
  ...
  // Step 2: Try to find account by email (fallback)
  if (!accountId && session.customer_details && session.customer_details.email) {
    ...
    const { data: membersByEmail, error: emailSearchError } = await supabase
      .from('members')
      .select('account_id, member_id, first_name, last_name, status, membership')
...
testLatestWebhookEvent().catch(console.error); 