-- Migration to fix time formatting issue in schedule_reservation_reminders function
-- The issue was that send_time (TIME type) was being concatenated incorrectly, creating malformed time strings

-- Fix the schedule_reservation_reminders function
...
-- Fix the update_reminder_times_on_reservation_update function
...
COMMENT ON FUNCTION schedule_reservation_reminders IS 'Fixed version that properly handles TIME type send_time without malformed concatenation';
COMMENT ON FUNCTION update_reminder_times_on_reservation_update IS 'Fixed version that properly handles TIME type send_time without malformed concatenation'; 