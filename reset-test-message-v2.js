...
async function resetTestMessage() {
  console.log('🔄 Resetting test message with enhanced debugging...');
  ...
  // Update the message to send in 3 minutes from now
  const now = new Date();
  const threeMinutesFromNow = new Date(now.getTime() + 3 * 60 * 1000);
  const timeString = threeMinutesFromNow.toTimeString().slice(0, 5); // HH:MM format
  ...
  if (updateError) {
    ...
    console.log('🎯 Ready to test! The message will send in about 3 minutes.');
    console.log('🔍 Enhanced debugging will show detailed processing information.');
  }
