...
import { format, subMonths, startOfMonth, endOfMonth, addDays, addMonths, isBefore } from 'date-fns';
  ...
  const msg = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <p><strong>Important:</strong> Your membership renewal is coming up soon. Please review your statement and ensure any outstanding balance is paid before your renewal date.</p>
...
export default async function handler(req, res) {
  ...
  try {
    const { member_id, account_id, month, year, send_all } = req.body;
    ...
    if (send_all) {
      ...
      console.log(`Found ${members.length} members with renewal date tomorrow`);
    } else {
      // Get specific member
      ...
      if (member_id) {
        ...
        return res.status(400).json({ error: 'member_id, account_id, or send_all is required' });
      }
    ...
    return res.status(500).json({ error: 'Failed to send statements' });
  }
}