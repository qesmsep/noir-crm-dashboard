...
const res = (start: string, end: string, party: number | null) => ({
...
describe('calcPeakConcurrentGuests', () => {
  ...
  it('sums party sizes of reservations that overlap each other', () => {
    const reservations = [
      res('18:00', '20:00', 10),
      res('18:30', '20:30', 20),
      res('19:00', '21:00', 30),
  ...
  it('takes the peak, not the total, when reservations do not overlap each other', () => {
  ...
  it('ignores null party sizes and invalid dates without throwing', () => {
    const reservations = [
      res('18:00', '20:00', null),
      { start_time: 'not-a-date', end_time: 'also-bad', party_size: 99 },
      res('18:00', '20:00', 8),
    ];
...
describe('isCapacityError', () => {
  it('detects the trigger error by its marker', () => {
    expect(
      isCapacityError({ message: `${CAPACITY_ERROR_MARKER}: this reservation would put 104 guests on site at once (limit is 100)` })
    ).toBe(true);
  });
