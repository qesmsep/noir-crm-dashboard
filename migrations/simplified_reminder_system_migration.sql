-- Simplified Reminder System Migration
-- This updates the reservation_reminder_templates table to use a more flexible structure

-- Step 1: Add new columns to reservation_reminder_templates
...
-- Step 1.5: Drop old columns if they exist (for backward compatibility)
ALTER TABLE public.reservation_reminder_templates 
DROP COLUMN IF EXISTS reminder_type,
DROP COLUMN IF EXISTS send_time,
DROP COLUMN IF EXISTS send_time_minutes;

-- Step 1.6: Drop old enum types if they exist (for backward compatibility)
DROP TYPE IF EXISTS reminder_type CASCADE;

-- Step 2: Update existing templates to use the new structure
...
-- Step 3: Drop existing view and recreate it
...
-- Step 4: Drop existing functions and recreate them
...
-- Step 4.5: Create a function to check if a template should be sent
...
-- Step 5: Create a function to send template messages
